...
async function run() {
    ...
    if (github.context.eventName === "issue_comment") {
        ...
        if (authorAssociation !== "COLLABORATOR" && authorAssociation !== "OWNER" && authorAssociation !== "MEMBER" && authorAssociation !== "CONTRIBUTOR") {
            console.log(`skipped due to authorAssociation: ${authorAssociation}`);
            core.setOutput("triggered", "false");
            return;
        }

        const pull_request = unwrapResult(await octokit.request('GET /repos/{owner}/{repo}/pulls/{pull_number}', {
        ...
        if (debug) {
            console.log(`Pull request: ${JSON.stringify(pull_request, undefined, 2)}`);
        }

        const check_runs = unwrapResult(await octokit.request('GET /repos/{owner}/{repo}/commits/{ref}/check-runs', {
            owner: github.context.payload.repository.owner.login,
            repo: github.context.payload.repository.name,
        ...
        for (let i = 0; i < check_runs.length; i++) {
            ...
            if (check.conclusion === "failure") {
                if (debug) {
                    console.log(`Check: ${JSON.stringify(check, undefined, 2)}`);
                }

                const job = unwrapResult(await octokit.request('GET /repos/{owner}/{repo}/actions/jobs/{job_id}', {
                    owner: github.context.payload.repository.owner.login,
                    repo: github.context.payload.repository.name,
                    job_id: check.id
                }))
                ...
                unwrapResult(await octokit.request('POST /repos/{owner}/{repo}/actions/runs/{run_id}/rerun-failed-jobs', {
                    owner: github.context.payload.repository.owner.login,
                    repo: github.context.payload.repository.name,
                    run_id: job.run_id
                }))
